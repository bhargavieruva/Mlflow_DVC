...
def create_model(input_shape, optimizer, loss, metrics):
    ...
    x = keras.layers.Rescaling(1. / config.model_config_.scaling_factor)(x)
...
# Create model
classifier = create_model(input_shape = config.model_config_.input_shape, 
                          optimizer = config.model_config_.optimizer, 
                          loss = config.model_config_.loss, 
                          metrics = [config.model_config_.accuracy_metric])
